...
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		...
		*)
		...
		wget https://raw.githubusercontent.com/wahasa/Debian/main/Rootfs/debian-bookworm-${archurl}.sh && chmod +x debian-bookworm-${archurl}.sh && ./debian-bookworm-${archurl}.sh
	fi
	cur=`pwd`
	mkdir -p "$folder"
	cd "$folder"
