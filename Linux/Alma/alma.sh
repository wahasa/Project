...
if [ "$first" != 1 ];then
        if [ ! -f $tarball ]; then
                ...
                *)
                ...
                wget "https://github.com/AlmaLinux/docker-images/blob/d50d0f46d539381c5360a314dbac1b04135fff16/almalinux-9-docker-${archurl}-base.tar.xz?raw=true" -O $tarball
        fi
        cur=`pwd`
        mkdir -p "$folder"
        cd "$folder"
