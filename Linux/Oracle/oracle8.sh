...
if [ "$first" != 1 ];then
        if [ ! -f $tarball ]; then
                ...
                *)
                ...
                wget "https://github.com/oracle/container-images/blob/b42a4ac92cf22a1b724b3f44fcfd5af9fe19e7f5/8/oraclelinux-8-${archurl}-rootfs.tar.xz?raw=true" -O $tarball
                wget "https://github.com/oracle/container-images/blob/1947e98c8d9e0b520d96a7ceb052f63eeeb8105c/8/oraclelinux-8-${archurl}-rootfs.tar.xz?raw=true" -O $tarball
        fi
        cur=`pwd`
        mkdir -p "$folder"
        cd "$folder"
